...
class MediaSplit extends EventEmitter {
  ...
  _handleUrl () {
    return ytdl.getInfo(this._options.input).then((info) => {
      const downloadOptions = { quality: 'highestaudio', filter: this._options.audioonly ? 'audioonly' : undefined }
      ...
      return this._checkDownloadCache(this._inputFile, format, info)
    })
  }
  ...
  _checkDownloadCache (file, format, info) {
  ...
  _isUrl (input) {
    return (typeof input === 'string') && input.match(urlRegex)
  }
