...
class MediaSplit extends EventEmitter {
  ...
  _downloadCover (info) {
    ...
    const url = `https://img.youtube.com/vi/${info.video_id}/maxresdefault.jpg`
    return new Promise((resolve) => {
      const outputFile = path.join(this._options.output, 'cover.jpg')
      const getRequest = request(url)
 ...
 * @param {object} - The video info
 */

/**
 * Data event
 * @event MediaSplit#data
 * @param {object[]} - Array with the parsed sections
 */
 ...
 * @param {object} - Section info
 * @param {number} - Section index
 */

/**
 * Download progress
 * @event MediaSplit#downloadProgress
 * @param {number} - Chunk length in bytes
 * @param {number} - Total downloaded in bytes
 * @param {number} - Total download length in bytes
 */
